...
object AccumulatorDemo {
  ...
  def collectionAccDemo(sc:SparkContext) :Unit = {
    ...
    sc.register(collectionAcc, "Strings having more than 10 letters")
    val rdd4 = sc.textFile("C:\\student.txt")
    def collAdd(x:String) : Unit = { if ( x.length > 10 ) {collectionAcc.add(x) ; println(x)}}
  ...
  def longAccDemo(sc: SparkContext ) :Unit = {
    ...
    val longAcc = sc.longAccumulator("Count of words having more than 10 letters")
    val rdd4 = sc.textFile("C:\\student.txt")
    def collAdd(x:String) : Unit = { if ( x.length > 10 ) {longAcc.add(1) ; println(x)}}
    rdd4.flatMap(x => x.split(" ")).foreach( x => collAdd(x))
